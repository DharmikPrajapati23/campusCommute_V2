...
adminRouter.post("/admin/signup", async (req, res) => {
  try {
    ...
    // ✅ Validate password strength HERE before hashing (not in schema)
    ...
    const newAdmin = new Admin({ id, password: passwordHash, name });
    await newAdmin.save();
...
adminRouter.post("/admin/login", async (req, res) => {
  try {
    ...
    const isPasswordCorrect = await findAdmin.validatePassword(password);
    if (!isPasswordCorrect) {
      return res.status(401).json({ message: "Invalid Password" });
    }

    const token = await findAdmin.getJWT();

    // ✅ Return token in body — no cookie (blocked cross-origin on Render)
    res.status(200).json({
...
adminRouter.post("/admin/logout", (req, res) => {
  // Token lives in localStorage on client — nothing to clear server-side
...
adminRouter.get("/admin/getbuses", adminAuth, async (req, res) => {
  try {
    ...
    const buses = await Bus.find({ city });
    if (buses.length === 0) return res.status(404).json({ message: "No buses found for the specified city" });
...
adminRouter.post("/admin/addbuses", adminAuth, async (req, res) => {
  try {
    ...
    const newBus = new Bus({ busNumber, source, destination, city, departureTime, arrivalTime });
    await newBus.save();

    res.status(201).json({ message: "Bus saved successfully!" });
  } catch (error) {
    res.status(500).json({ message: "Internal Server Error" });
  }
});
...
adminRouter.put("/admin/updateCollege/:id", adminAuth, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error updating college: " + error.message });
  }
});

adminRouter.delete("/admin/deleteCollege/:id", adminAuth, async (req, res) => {
  try {
    const deletedCollege = await CollegeDetails.findByIdAndDelete(req.params.id);
    if (!deletedCollege) return res.status(404).json({ message: "College not found" });
    res.json({ message: "College deleted successfully" });
  } catch (error) {
    res.status(500).json({ message: "Error deleting college: " + error.message });
  }
});
